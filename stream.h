...
TL_INLINE u16 tl_bs_pull16_def0(tl_byte_source_pullable* self)
{
	u8 x = tl_bs_pull_def0(self);
	return (x << 8) + tl_bs_pull_def0(self);
}

TL_INLINE u32 tl_bs_pull32_def0(tl_byte_source_pullable* self)
{
	u16 x = tl_bs_pull16_def0(self);
	return (x << 16) + tl_bs_pull16_def0(self);
}

TL_INLINE u16 tl_bs_pull16_def0_le(tl_byte_source_pullable* self)
{
	u8 x = tl_bs_pull_def0(self);
	return (tl_bs_pull_def0(self) << 8) + x;
}

TL_INLINE u32 tl_bs_pull32_def0_le(tl_byte_source_pullable* self)
{
	u16 x = tl_bs_pull16_def0_le(self);
...
TL_STREAM_API int  tl_bs_pulln(tl_byte_source_pullable* src, u8* data, int n);
...
TL_INLINE void tl_bs_push16(tl_byte_sink_pushable* self, u16 x)
{
	tl_bs_push(self, (x>>8));
	tl_bs_push(self, x & 0xff);
}

TL_INLINE void tl_bs_push32(tl_byte_sink_pushable* self, u32 x)
{
	tl_bs_push16(self, (x>>16) & 0xffff);
	tl_bs_push16(self, x & 0xffff);
}

TL_INLINE void tl_bs_push16_le(tl_byte_sink_pushable* self, u16 x)
{
	tl_bs_push(self, x & 0xff);
	tl_bs_push(self, (x>>8));
}

TL_INLINE void tl_bs_push32_le(tl_byte_sink_pushable* self, u32 x)
{
	tl_bs_push16_le(self, x & 0xffff);
	tl_bs_push16_le(self, (x>>16) & 0xffff);
}

TL_STREAM_API int    tl_bs_seek_sink(tl_byte_sink_pushable* self, u64 p);
TL_STREAM_API int    tl_bs_pushn(tl_byte_sink_pushable* self, u8 const* data, int n);
TL_STREAM_API u64 tl_bs_tell_sink(tl_byte_sink_pushable* self);
